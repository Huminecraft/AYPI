	...
	public String compile() {
		...
		for (int i = 0 ; i < args.length ; i++) {
			...
			if (sm.isMathCalculator(word)) {
				...
				if (sm.isNumber(last) && sm.isNumber(next)) {
					replaceCode(last+" "+word+" "+next, sm.doCalcule(last, next, word, line, codeOrigine));
					i = 0;
				} else if (sm.getMathOperator(word) == MathOperator.ADDITION) {
				...
				if (sm.isNumber(last) && sm.isNumber(next)) {
					...
					i = 0;
				} else if (sm.getMathOperator(word) == MathOperator.EQUALS) {
				...
				if (sm.isBoolean(last) && sm.isBoolean(next)) {
