...
public class Square {
		...
		for (int y = 0; pos1.getBlockY() + y <= pos2.getBlockY(); y++) {
			for (int x = 0; pos1.getBlockX() + x <= pos2.getBlockX(); x++) {
				for (int z = 0; pos1.getBlockZ() + z <= pos2.getBlockZ(); z++) {
					...
					locations.add(new Location(pos1.getWorld(), pos1.getBlockX() + x, pos1.getBlockY() + y,
							pos1.getBlockZ() + z));
	...
	public boolean containLocation(Location loc) {
		if (this.getPos1().getBlockX() <= loc.getBlockX()
		&& this.getPos1().getBlockY() <= loc.getBlockY()
		&& this.getPos1().getBlockZ() <= loc.getBlockZ()
		&& this.getPos2().getBlockX() >= loc.getBlockX()
		&& this.getPos2().getBlockY() >= loc.getBlockY()
		&& this.getPos2().getBlockZ() >= loc.getBlockZ()) {
			return true;
		} else {
			return false;
		}
